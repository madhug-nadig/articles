...
Now that we have finished up the algorithm, it's time to test how well it performs. Since for this dataset, k-NN is a bondary classifier, I will be using [classification accuracy](https://en.wikipedia.org/wiki/Accuracy_and_precision#In_binary_classification) to evaluate the algorithm. 
...
Now, let's compare our implmentation with the sci-kit learn implementation. This is just for demonstration purposes only. Tf I was using k-NN algorithm in a production environment, I would _definitely_ use the library function and so should you. 
...
However, in most runs, the accuracy hovered around the value `81.25`. I was quite surprised at the result. I am not fully sure as to why the custom implementation out-performed the sci-kit learn implementation. This probably has something to do with the fact that I have used sci-kit k-NN as it is - without any customization whatsoever. The _k_ value and _distance metric_ themselves play an important role in accuracy. It is also possible that sci-kit implementation refrains from going through the entire dataset to improve the running time.
